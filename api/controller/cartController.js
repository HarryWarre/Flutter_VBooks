...
module.exports = {
    createCart: async (req, res) => {
        ...
        try{
            ...
            res.status(200).json({message: "Tạo giỏ hàng thành công", success: true, cart: saveCart} )
        }catch(err){
    ...
    getCart: async (req, res) => {
        await Cart.find().then(info => res.json(info)).catch(err => res.json(err))
    },
    ...
    editCart: async (req, res) => {
        ...
        try {
            ...
            res.status(200).json({ success: true, message: "Cập nhật thành công", cart: updatedCart });
        } catch (err) {
            res.status(500).json(err);
        }
    },
    ...
    deleteCart: async (req, res) => {
        ...
        try {
            ...
            res.status(500).json({ success: false, message: 'Xoá giỏ hàng thất bại', error: error.message });
        }
    }
